...
-- Note: Service role will bypass RLS, so API key authentication works
...
-- Allow service role to read API keys for authentication
-- This is needed for API key authentication to work
DROP POLICY IF EXISTS "Service role can read api_keys" ON api_keys;
CREATE POLICY "Service role can read api_keys" ON api_keys
...
DROP POLICY IF EXISTS "Service role can read transactions" ON wallet_transactions;
CREATE POLICY "Service role can read transactions" ON wallet_transactions
  FOR SELECT USING (true);
...
-- Updated for Supabase Auth schema (uses user_profiles table)
...
BEGIN
  ...
  IF current_balance IS NULL THEN
    ...
    INSERT INTO user_profiles (id, wallet_balance)
    VALUES (p_user_id, 1000.00)
    ON CONFLICT (id) DO UPDATE SET wallet_balance = 1000.00;
  END IF;
  ...
  UPDATE user_profiles SET wallet_balance = new_balance WHERE id = p_user_id;
...
-- Only update user_profiles since we're using Supabase Auth
UPDATE user_profiles SET wallet_balance = 1000.00 WHERE wallet_balance IS NULL;

-- Also initialize for any users that don't have a profile yet
INSERT INTO user_profiles (id, wallet_balance)
SELECT id, 1000.00
FROM auth.users
